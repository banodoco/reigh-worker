...
def test_wan_2_2_t2i_uses_single_frame_scratchpad(monkeypatch, tmp_path):
    ...
    assert 'workflow.nodes[\'16\'].inputs[\'negative_prompt\'] = "fading, breaking, shot cuts, jumpcuts, blurry, noise, distorted"' in source
    assert "workflow.nodes['27'].inputs['cfg'] = 3.0" in source
    assert "workflow.nodes['27'].inputs['shift'] = 5.0" in source
    assert "workflow.nodes['27'].inputs['seed'] = 2026" in source
    assert "workflow.nodes['27'].inputs['widget_3'] = 2026" in source
    assert "workflow.nodes['87'].inputs['cfg'] = 1.0" in source
    assert "workflow.nodes['87'].inputs['shift'] = 5.0" in source
    assert "workflow.nodes['87'].inputs['seed'] = 2026" in source
...
def test_wan_vace_materializes_anchors_control_and_scratchpad(monkeypatch, tmp_path):
    ...
    assert "workflow.nodes['139'].inputs['format'] = 'video/h264-mp4'" in source
    assert "workflow.nodes['139'].inputs['pingpong'] = False" in source
    assert 'workflow.nodes[\'16\'].inputs[\'widget_0\'] = "vace prompt"' in source
    assert 'workflow.nodes[\'16\'].inputs[\'positive_prompt\'] = "vace prompt"' in source
    assert "workflow.nodes['27'].inputs['cfg'] = 3.0" in source
    assert "workflow.nodes['87'].inputs['cfg'] = 1.0" in source
